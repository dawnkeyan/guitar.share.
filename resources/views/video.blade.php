    ...
    <div class="container">
        ...
        <table class="table">
            ...
            @foreach ($videos as $item)
                <tr>
                    <td><a href="/comment?category_id={{ $item->id }}&category=videos">{{ $item->name }}</a></td>
                    <td><a href="/comment?category_id={{ $item->id }}&category=videos"><img src="{{ $item->cover }}" width="95" height="84"></a></td>
                    <td><a href="/comment?category_id={{ $item->id }}&category=videos">{{ $item->created_at }}</a></td>
                    {{--<td><a href="{{ $item->file }}">观看</a></td>--}}
                    <td><a href="/comment?category_id={{ $item->id }}&category=videos">{{ $item->context }}</a></td>
                    <td>
...
<script>
    function search() {
        ...
        window.location.assign('/video?name=' + name + '&start_time=' + start_time + '&end_time=' + end_time);
    }
